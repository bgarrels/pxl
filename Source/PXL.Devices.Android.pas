...
//      LogText('Resulting height is: ' + IntToStr(NewContentRect.Height));
...
var
  ...
  EventType, EventAction, PointerIndex, PointerCount, I: LongInt;
  EventStack: array of LongInt;
begin
  ...
  begin
    ...
    PointerIndex := (EventAction and AMOTION_EVENT_ACTION_PointerIndex_MASK) shr
    ...
    // Create and fill event stack that will be provided to message receivers.
    SetLength(EventStack, 2 + PointerCount * 2);

    EventStack[0] := PointerCount;
    EventStack[1] := PointerIndex;

    for I := 0 to PointerCount - 1 do
    begin
      EventStack[2 + I * 2] := Round(AMotionEvent_getX(Event, I));
      EventStack[3 + I * 2] := Round(AMotionEvent_getY(Event, I));
    end;

    try
{      case EventAction of
        AMOTION_EVENT_ACTION_DOWN:
          EventTouchDown.Notify(Application, @EventStack[0], False, False);

        AMOTION_EVENT_ACTION_MOVE:
          EventTouchMove.Notify(Application, @EventStack[0], False, False);

        AMOTION_EVENT_ACTION_UP:
          EventTouchUp.Notify(Application, @EventStack[0], False, False);
